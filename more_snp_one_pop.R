...
##
## this is adding larger SNP panels
##
library(tidyverse)
library(gRandma)
library(EFGLmh)
...
for(i in 1:999){
	...
	if(i == 499){
		gmaIn_snp_500 <- gmaIn_snp
	}
	if(i == 599){
		gmaIn_snp_600 <- gmaIn_snp
	}
	if(i == 699){
		gmaIn_snp_700 <- gmaIn_snp
	}
	if(i == 799){
		gmaIn_snp_800 <- gmaIn_snp
	}
	if(i == 899){
		gmaIn_snp_900 <- gmaIn_snp
	}
}
gmaIn_snp_1000 <- gmaIn_snp

print("false negative")

# false negative rates

fn_snp <- mclapply(list(gmaIn_snp_500, gmaIn_snp_600, gmaIn_snp_700, gmaIn_snp_800, gmaIn_snp_900, gmaIn_snp_1000),
						 falseGrandma, relationship = "ssGP", llrToTest = seq(-10,80,1), seed = 7,
						 N = 10000, errorType = "falseNeg", mc.cores = numCores)

print("false positive IS")

system.time(
	fp_unrel_snp_IS <- mclapply(list(gmaIn_snp_500, gmaIn_snp_600, gmaIn_snp_700, gmaIn_snp_800, gmaIn_snp_900, gmaIn_snp_1000),
										 falseGrandma, relationship = "ssGP", llrToTest = seq(-10,80,1), seed = 7,
										 N = 1000000, errorType = "Unrel", method = "IS", mc.cores = numCores)
)

save.image("extra_snp_unrel_IS_only.rda") # to get important results early

print("false positive strat")

# false positive unrel rates
system.time(
	fp_unrel_snp <- mclapply(list(gmaIn_snp_500, gmaIn_snp_600, gmaIn_snp_700, gmaIn_snp_800, gmaIn_snp_900, gmaIn_snp_1000),
									 falseGrandma, relationship = "ssGP", llrToTest = seq(-10,80,1), seed = 7,
									 itersPerMI = rep(1000000,20), errorType = "Unrel", mc.cores = numCores)
)

save.image("extra_snp_unrel.rda")
