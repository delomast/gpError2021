...
# this is error for other relationships
...
tempFunc <- function(rel, method, seed){
		return(
			falseGrandma(gmaIn_mh_300, relationship = "ssGP", llrToTest = seq(-10,40,1), seed = seed,
							 itersPerMI = rep(1000000,20), errorType = rel, N = 1000000,
							 method = method)
		)
	}
set.seed(7)
randSeeds <- sample.int(.Machine$integer.max, 2*length(relsToTest))
system.time(
	fp_mh <- mcmapply(tempFunc, c(relsToTest, relsToTest), c(rep("IS", length(relsToTest)), rep("strat", length(relsToTest))),
							randSeeds, mc.cores = numCores)
)

save.image("diffRel.rda")
